import { UrlTile } from 'react-native-maps'

export const Albums = [
    {
        id: 1,
        name: '30',
        artist: 'Adele',
        year: '2021',
        price: 40.98,
        cover: require('../assets/30.png'),
        songs: [
            {
                id: 1,
                name: 'Easy On Me',
                duration: '3:44',
                url: 'https://drive.google.com/uc?export=download&id=1MM5RJZHMnRJ8hRscTZYnzGYCwdycbi19',
            },
            {
                id: 2,
                name: 'Strangers by Nature',
                duration: '3:02',
            },
            {
                id: 3,
                name: 'My Little Love',
                duration: '6:29',
            },
            {
                id: 4,
                name: 'Cry Your Heart Out',
                duration: '4:15',
            },
            {
                id: 5,
                name: "Oh My God",
                duration: '3:45',
            },
            {
                id: 6,
                name: "Can I Get It",
                duration: '3:30',
            },
            {
                id: 7,
                name: "I Drink Wine",
                duration: '6:16',
            },
            {
                id: 8,
                name: "All Night Parking",
                duration: '2:41',
            },
            {
                id: 9,
                name: 'My Little Love',
                duration: '6:29',
            },
            {
                id: 10,
                name: "Love Is a Game",
                duration: '6:43',
            },
            {
                id: 11,
                name: "Hold On",
                duration: '6:06',
            },
            {
                id: 12,
                name: "To Be Loved",
                duration: '6:43',
            },
            {
                id: 13,
                name: "Wild Wild West",
                duration: '3:46',
            },
            {
                id: 14,
                name: "Can't Be Together",
                duration: '4:18',
            },
            {
                id: 15,
                name: "Easy on Me (with Chris Stapleton)",
                duration: '3:44',
            },
        ],
        description: "30 is the fourth studio album by English singer-songwriter Adele, released on 19 November 2021 by Columbia Records. Musically, 30 is a pop, soul, and jazz record. It has been described as Adele's most creative work sonically, expanding on her past works by incorporating dance-pop and gospel elements. The album reached number one in 24 countries, including the UK where it became Adele's fourth consecutive number-one album, making her the first act to debut all their albums atop the UK Albums chart; it garnered the highest first-week sales for an album by a female artist in the UK since Adele's own 25 and became 2021's best-seller in the country. In the US, it was Adele's third consecutive Billboard 200 number-one album, and the year's best-seller as well. 30 was the world's best-selling album of 2021, with over 5 million copies sold in the year. Additionally, Adele was the third best-selling artist of 2021 globally. 30 won British Album of the Year at the 42nd Brit Awards, making Adele the first solo artist in history to win the honour three times."
    },

    {
        id: 2,
        name: '1989',
        artist: 'Taylor Swift',
        year: '2014',
        price: 60.79,
        cover: require('../assets/1989.jpg'),
        songs: [
            {
                id: 1,
                name: 	"Blank Space",
                duration: '3:52',
                url: 'https://drive.google.com/uc?export=download&id=1CGeiZhsFfXsmAyrEUAsAD9dd9gKv6Vj1',
            },
            {
                id: 2,
                name: "Shake It Off",
                duration: '3:39',
            },
            {
                id: 3,
                name: 	"Welcome to New York",
                duration: '3:33',
            },
            {
                id: 4,
                name: "Style",
                duration: '3:51',
            },
            {
                id: 5,
                name: "Out of the Woods",
                duration: '3:56',
            },
            {
                id: 6,
                name: "All You Had to Do Was Stay",
                duration: '3:13',
            },
            {
                id: 7,
                name: "I Wish You Would",
                duration: '3:27',
            },
            {
                id: 8,
                name: "Bad Blood",
                duration: '3:32',
            },
            {
                id: 9,
                name: "How You Get the Girl",
                duration: '4:08',
            },
            {
                id: 10,
                name: "Love Is a Game",
                duration: '6:43',
            },
            {
                id: 11,
                name: "This Love",
                duration: '4:10',
            },
            {
                id: 12,
                name: "I Know Places",
                duration: '3:16',
            },
            {
                id: 13,
                name: "Clean",
                duration: '4:31',
            },
            {
                id: 14,
                name: "Wonderland",
                duration: '4:05',
            },
            {
                id: 15,
                name: "You Are in Love",
                duration: '4:27',
            },
            {
                id: 16,
                name: "New Romantics",
                duration: '3:50',
            },
            {
                id: 17,
                name: "I Know Places",
                duration: '3:36',
            },
            {
                id: 18,
                name: "I Wish You Would",
                duration: '1:47',
            },
            {
                id: 19,
                name: "Blank Space",
                duration: '2:11',
            },
        ],
        description: "1989 is Taylor Swift's fifth studio album. It was released on October 27, 2014, and features 13 songs. Swift named the album 1989 because, in addition to being the year in which she was born, the late '80s also influenced the music on her album. This album won 3 Grammy Awards including Album of the Year at the 58th Grammy Awards on February 15, 2016."
    },

    {
        id: 3,
        name: 'Lust for Life',
        artist: 'Lana Del Rey',
        year: '2017',
        price: 47.76,
        cover: require('../assets/lustforlife.jpg'),
        songs: [
            {
                id: 1,
                name: "Love",
                duration: '4:32',
                url: 'https://drive.google.com/uc?export=download&id=196EVHgVyMf39YLa5ZDIrVYZqLLKEv98o',
            },
            {
                id: 2,
                name: "Lust for Life",
                duration: '4:24',
            },
            {
                id: 3,
                name: "13 Beaches",
                duration: '4:55',
            },
            {
                id: 4,
                name: "Cherry",
                duration: '3:00',
            },
            {
                id: 5,
                name: "White Mustang",
                duration: '2:44',
            },
            {
                id: 6,
                name: "Summer Bummer",
                duration: '4:20',
            },
            {
                id: 7,
                name: "Groupie Love",
                duration: '4:24',
            },
            {
                id: 8,
                name: "In My Feelings",
                duration: '3:58',
            },
            {
                id: 9,
                name: "Coachella – Woodstock in My Mind",
                duration: '4:18',
            },
            {
                id: 10,
                name: "God Bless America – and All the Beautiful Women in It",
                duration: '4:36',
            },
            {
                id: 11,
                name: "When the World Was at War We Kept Dancing",
                duration: '4:35',
            },
            {
                id: 12,
                name: "Beautiful People Beautiful Problems",
                duration: '4:13',
            },
            {
                id: 13,
                name: "Tomorrow Never Came",
                duration: '5:07',
            },
            {
                id: 14,
                name: "Heroin",
                duration: '5:55',
            },
            {
                id: 15,
                name: "Change",
                duration: '5:21',
            },
            {
                id: 16,
                name: "Get Free",
                duration: '5:34',
            },
        ],
        description: 'Lust for Life is the fifth studio album by American singer-songwriter Lana Del Rey, released on July 21, 2017, through Polydor and Interscope worldwide and Urban in Germany. Marking a return to the "hip-hop inspired" sound of her major-label debut, Lust for Life features production from past collaborators Rick Nowels, Kieron Menzies and Emile Haynie, while also working for the first time with producers Boi-1da, Max Martin, Benny Blanco and Metro Boomin. Lust for Life received positive reviews from music critics and was nominated for a Grammy Award in the category of Best Pop Vocal Album at the 60th Annual Grammy Awards, becoming Del Rey\'s second nomination in the category. The album achieved commercial success, debuting at number one on the US Billboard 200 chart, becoming her second number-one album.'
    },
    {
        id: 4,
        name: 'Hoang',
        artist: 'Hoang Thuy Linh',
        year: '2019',
        price: 42.89,
        cover: require('../assets/hoang.jpg'),
        songs: [
            {
                id: 1,
                name: "Duyen am",
                duration: '3:00',
                url: 'https://drive.google.com/uc?export=download&id=1U1eEFHjyMDWyUMuEFWC0BMtSbvPD-23-',
            },
            {
                id: 2,
                name: "De Mi noi cho ma nghe",
                duration: '3:14',
            },
            {
                id: 3,
                name: "Khoi đau",
                duration: '0:35',
            },
            {
                id: 4,
                name: "Em day chang phai Thuy Kieu",
                duration: '3:35',
            },
            {
                id: 5,
                name: "Lam moi toi ngoi khong",
                duration: '3:15',
            },
            {
                id: 6,
                name: "Keo ca keo ket",
                duration: '3:16',
            },
            {
                id: 7,
                name: "Banh troi nuoc",
                duration: '4:30',
            },
            {
                id: 8,
                name: "Tu phu",
                duration: '4:25',
            },
            {
                id: 9,
                name: "Ke cap gap ba gia" ,
                duration: '3:52',
            },
            {
                id: 10,
                name: "Giai ket",
                duration: '1:16',
            },
        ],
        description: 'Hoang is the third studio album by Vietnamese female singer Hoang Thuy Linh , released by Thoi Dai Records and Sony Music Entertainment on October 20, 2019. The album includes 9 songs in the future bass genre, electronic folk songs, global music. The album brought home many awards, including the Dedication Music Award for "Album of the Year", helping DTAP as well as Linh herself win the categories "Producer of the year" and "Singer of the year". Hoang is a theme album imbued with Vietnamese folklore and literature, it is also the emotional journey of a woman going through many ups and downs in life. Therefore, "Hoang" is not only appreciated for the quality of the music, but also the melody and lyrics have received positive feedback.'
    },
    {
        id: 5,
        name: 'Blue Neighbourhood',
        artist: 'Troye Sivan',
        year: '2015',
        price: 35.17,
        cover: require('../assets/blueneigh.jpg'),
        songs: [
            {
                id: 1,
                name: "Youth",
                duration: '3:05',
                url: 'https://drive.google.com/uc?export=download&id=1GSd6WSAYVPl6GycuDySQp1gpsjQh74g_',
            },
            {
                id: 2,
                name: "Fools",
                duration: '3:40',
            },
            {
                id: 3,
                name: "Ease",
                duration: '3:33',
            },
            {
                id: 4,
                name: "Bite",
                duration: '3:06',
            },
            {
                id: 5,
                name: "The Quiet",
                duration: '3:46',
            },
            {
                id: 6,
                name: "DKLA",
                duration: '4:15',
            },
            {
                id: 7,
                name: "Talk Me Down",
                duration: '3:57',
            },
            {
                id: 8,
                name: "Cool",
                duration: '3:21',
            },
            {
                id: 9,
                name: "Heaven",
                duration: '4:21',
            },
            {
                id: 10,
                name: "Wild",
                duration: '3:47',
            },
            {
                id: 11,
                name: "Lost Boy",
                duration: '3:43',
            },
            {
                id: 12,
                name: "For Him.",
                duration: '3:29',
            },
            {
                id: 13,
                name: "Suburbia",
                duration: '3:53',
            },
            {
                id: 14,
                name: "Too Good",
                duration: '3:44',
            },
            {
                id: 15,
                name: "Blue",
                duration: '3:31',
            },
            {
                id: 16,
                name: "Swimming Pools",
                duration: '3:26',
            },
            {
                id: 17,
                name: "Happy Little Pill",
                duration: '4:02',
            },
        ],
        description: "Blue Neighbourhood is the debut studio album by Australian singer and songwriter Troye Sivan. It was released internationally on 4 December 2015 via EMI Music Australia and Capitol Records America. Blue Neighbourhood has been described to be an electropop, gloom-pop, and synth-pop record with EDM, electronic music, electro, and R&B elements. Lyrical themes within the record include Sivan's experiences with love, youth, yearning, heartbreak, and affirmation."
    },
]

export const Cart = {
    list: [],
}

export const Stores = [
    { 
        key: 1,
        coordinates: { latitude: 21.030961775351827, longitude: 105.84854823005392 },
        storeName: 'Aurora in Hanoi',
        address: '23, Cotton street, Hang Gai ward, Hoan Kiem district, Ha Noi city',
    },
    { 
        key: 2,
        coordinates: { latitude: 16.06336904296533, longitude:  108.23408680558316 },
        storeName: 'Aurora in Danang',
        address: '965, Ngo Quyen street, An Hai Bac ward, Son Tra district, Da Nang city',

    },
    { 
        key: 3,
        coordinates: { latitude: 10.7748190093447, longitude:  106.69986300331179 },
        storeName: 'Aurora in Ho Chi Minh',
        address: '117, Le Thanh Ton street, Ben Nghe ward, District 1, Ho Chi Minh city',
    },
    { 
        key: 4,
        coordinates: { latitude: 10.38198089123573, longitude: 105.44246632357063 },
        storeName: 'Aurora in Long Xuyen',
        address: '21, Hai Ba Trung street, My Long ward, Long Xuyen city',
    },
  ];

export const Tour = [
    { 
        key: 1,
        name: 'Day Tour',
        cover: require('../assets/tour/day.jpg'),
        hours: '8.00 AM to 5.00 PM',
        priceAdult: 3.00,
        priceKid: 1.50, 


    },
    { 
        key: 2,
        name: 'Night Tour',
        cover: require('../assets/tour/night.jpg'),
        hours: '6.30 PM to 8.00 PM',
        priceAdult: 10.00,
        priceKid: 5.00, 
    },
];

export const Animals = [
    { 
        key: 1,
        name: 'Peacock',
        cover: require('../assets/animals/peacock.jpg'),
        icon: require('../assets/animals/icons/peacock.png'),
        coordinates: { latitude: 10.78747590103391, longitude: 106.70700385185006 },
    }, 
    { 
        key: 2,
        name: 'Flamingo',
        cover: require('../assets/animals/flamingo.jpg'),
        icon: require('../assets/animals/icons/flamingo.png'),
        coordinates: { latitude: 10.787624192038303, longitude: 106.70713232738176 },
    },
    { 
        key: 3,
        name: 'Elephant',
        cover: require('../assets/animals/elephant.jpg'),
        icon: require('../assets/animals/icons/elephant.png'),
        coordinates: { latitude: 10.788702272153307, longitude: 106.70696586282457 },
    },
    { 
        key: 4,
        name: 'Giraffe',
        cover: require('../assets/animals/giffare.jpg'),
        icon: require('../assets/animals/icons/giraffe.png'),
        coordinates: { latitude: 10.788530417130628, longitude: 106.70717829883604 },
    },
    { 
        key: 5,
        name: 'Tiger',
        cover: require('../assets/animals/tiger.jpg'),
        icon: require('../assets/animals/icons/tiger.png'),
        coordinates: { latitude: 10.785371698757286, longitude: 106.708165547904 },
    },
    { 
        key: 6,
        name: 'Crocodile',
        cover: require('../assets/animals/crocodile.jpg'),
        icon: require('../assets/animals/icons/crocodile.png'),
        coordinates: { latitude: 10.785905609701132, longitude: 106.70847702718977 },
    },
];

export const Souvenirs = [
    {
        id: 1,
        name: 'Wooden Pen',
        price: 3.00,
        cover: require('../assets/wooden_pen.jpg'),
        height: 4.5,
        width: 1.2,
    },
    {
        id: 2,
        name: '[Crochet] Giraffe',
        price: 15.00,
        cover: require('../assets/crochet_giffare.jpg'),
        height: 10,
        width: 3,
    },

    {
        id: 3,
        name: '[Crochet] Elephant',
        price: 15.00,
        cover: require('../assets/crochet_elephant.jpg'),
        height: 10,
        width: 3,
    },
    {
        id: 4,
        name: '[Crochet] Fox',
        price: 15.00,
        cover: require('../assets/crochet_fox.jpg'),
        height: 10,
        width: 3,
    }
]

